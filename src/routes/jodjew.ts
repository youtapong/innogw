import { Elysia, t } from "elysia";
import { sql } from "../db";

export const jodjewRoutes = new Elysia({ prefix: "/api/jodjew" })
  // 1. Background (Server to Server)
  .post(
    "/dev",
    async ({ body, request }) => {
      try {
        const clientIp = request.headers.get("x-forwarded-for") || request.headers.get("x-real-ip") || "unknown";
        const requestId = request.headers.get("x-request-id") || "";

        // Log the callback into api_logs
        await sql`
          INSERT INTO "api_logs" (api_name, request_body, x_client_ip, x_request_id, is_success)
          VALUES ('jodjew-callback-dev', ${JSON.stringify(body)}, ${clientIp}, ${requestId}, true)
        `;

        return { success: true, message: "Callback received" };
      } catch (error: any) {
        return { success: false, error: error.message };
      }
    },
    {
      body: t.Any(),
      detail: {
        tags: ["Payment - Jodjew"],
        summary: "Jodjew server-to-server callback (Dev)",
      },
    }
  )

  // 2. Foreground (Redirect success)
  .get(
    "/dev/success",
    async ({ query }) => {
      return {
        status: "success",
        message: "Payment processed successfully via Jodjew",
        transaction: query
      };
    },
    {
      detail: {
        tags: ["Payment - Jodjew"],
        summary: "Jodjew redirect success page (Dev)",
      },
    }
  )

  // 3. Foreground (Redirect fail)
  .get(
    "/dev/fail",
    async ({ query }) => {
      return {
        status: "fail",
        message: "Payment failed via Jodjew",
        transaction: query
      };
    },
    {
      detail: {
        tags: ["Payment - Jodjew"],
        summary: "Jodjew redirect fail page (Dev)",
      },
    }
  )

  // 4. Foreground (Redirect cancel)
  .get(
    "/dev/cancel",
    async ({ query }) => {
      return {
        status: "cancel",
        message: "Payment cancelled by user",
        transaction: query
      };
    },
    {
      detail: {
        tags: ["Payment - Jodjew"],
        summary: "Jodjew redirect cancel page (Dev)",
      },
    }
  );
