...
export const transactionRoutes = new Elysia({ prefix: "/transaction" })
  .post(
    ...
    async ({ headers, body, request, set }) => {
      ...
      const paymentType = (body as any)?.payment_type !== undefined ? (body as any).payment_type : 0;

      console.log(`[transaction-orderRef] Starting processing for orderRef: ${orderRef}, payment_type: ${paymentType}`);
      ...
      try {
        ...
        console.log(`[transaction-orderRef] Step 1: Splitting orderRef: ${orderRef}`);
        const parts = orderRef.split("-");
        const esCode = parts[0];
        console.log(`[transaction-orderRef] Step 1 complete. Extracted esCode: ${esCode}`);

        // 2. ค้นหาใน table product_mapping where es_code = value(0)
        console.log(`[transaction-orderRef] Step 2: Querying product_mapping for es_code: ${esCode}`);
        ...
        if (!mapping) {
          set.status = 404;
          const responseBody = { success: false, error: `Product mapping not found for esCode: ${esCode}` };
          await sql`
            INSERT INTO "api_logs" (api_name, request_body, response_body, order_ref, x_client_ip, x_request_id, is_success, error_message, status_code)
            VALUES ('transaction-orderRef-error', ${JSON.stringify(body || {})}, ${JSON.stringify(responseBody)}, ${orderRef}, ${clientIp}, ${requestId}, false, ${responseBody.error}, '404')
          `;
          return responseBody;
        }
        console.log(`[transaction-orderRef] Step 2 complete. Found mapping for esCode: ${esCode}`, JSON.stringify(mapping));

        // 3. ตรวจสอบ Authorization: Bearer token ที่ได้รับมา ถ้า token == product_token จึงจะทำงานต่อ
        console.log(`[transaction-orderRef] Step 3: Verifying Bearer token...`);
        if (token !== mapping.product_token) {
          set.status = 401;
          const responseBody = { success: false, error: "Unauthorized: Token mismatch or not found for the decoded esCode" };

          await sql`
            INSERT INTO "api_logs" (api_name, request_body, response_body, order_ref, x_client_ip, x_request_id, is_success, error_message, status_code)
            VALUES ('transaction-orderRef-unauthorized', ${JSON.stringify(body || {})}, ${JSON.stringify(responseBody)}, ${orderRef}, ${clientIp}, ${requestId}, false, 'Unauthorized: Token mismatch', '401')
          `;
          return responseBody;
        }
        console.log(`[transaction-orderRef] Step 3 complete. Token successfully verified.`);

        // 4. หาค่า "signature" จาก function @utils/crypto.ts
        console.log(`[transaction-orderRef] Step 4: Generating signature...`);
        ...
        console.log(`[transaction-orderRef] Step 4 complete. Generated signature: ${calculatedSignature}`);

        // 5. ตรวจสอบและบันทึกข้อมูลลูกค้า (Check/Insert Customer)
        console.log(`[transaction-orderRef] Step 5: Checking customer in custommer table...`);
        ...
        if (!existingCustomer) {
          console.log(`[transaction-orderRef] Step 5: Customer not found. Inserting new customer...`);
          ...
          console.log(`[transaction-orderRef] Step 5 complete. Inserted new customer with ID: ${customerDbId}`);
        } else {
          console.log(`[transaction-orderRef] Step 5: Customer found. Updating customer details with ID: ${existingCustomer.id}...`);
          await sql`
          ...
          console.log(`[transaction-orderRef] Step 5 complete. Updated customer details.`);
        }

        // 6. Update ลง table orders where order_ref
        console.log(`[transaction-orderRef] Step 6: Updating orders table for order_ref: ${orderRef}...`);
        await sql`
          UPDATE "orders" SET
        ...
        `;
        console.log(`[transaction-orderRef] Step 6 complete. Updated orders table.`);

        // 7. Insert ลง table order_items
        console.log(`[transaction-orderRef] Step 7: Inserting items into order_items...`);
        ...
        console.log(`[transaction-orderRef] Step 7 complete. Inserted all order items.`);

        // 8. สร้าง curl เพื่อส่งข้อมูล โดยนำค่าที่ได้ มาเติมใน ข้อมูลที่ส่ง NT-Eservie
        console.log(`[transaction-orderRef] Step 8: Preparing fetch payload for NT-Eservice...`);
        ...
        console.log(`[transaction-orderRef] Step 8: Sending POST request to ${fetchUrl}...`);
        ...
        console.log(`[transaction-orderRef] Step 8 complete. NT-Eservice response:`, fetchResult);
        ...
        if (mapping.bank_url) {
          console.log(`[transaction-orderRef] Step 9: Forwarding output to bank_url: ${mapping.bank_url}`);
          try {
            ...
            console.log(`[transaction-orderRef] Step 9 complete. Forward result:`, forwardResult);
          } catch (forwardErr: any) {
            console.error(`[transaction-orderRef] Step 9 failed. Failed to forward to bank_url:`, forwardErr.message);
          }
        } else {
          console.log(`[transaction-orderRef] Step 9: bank_url not configured. Skipping forward.`);
        }

        // 10. บันทึกข้อมูลการยิงข้อมูลชำระเงิน (Log to payment_logs)
        console.log(`[transaction-orderRef] Step 10: Logging to payment_logs...`);
        ...
        `;
        console.log(`[transaction-orderRef] Step 10 complete. Logged to payment_logs.`);

        // 11. ทุกขั้นตอน console.log & บันทึกประวัติธุรกรรม (Log to api_logs)
        console.log(`[transaction-orderRef] Step 11: Writing success log to api_logs...`);
        await sql`
        ...
        `;
        console.log(`[transaction-orderRef] Step 11 complete. Processing successful.`);
