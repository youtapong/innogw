...
export const transactionActionRoutes = new Elysia({ prefix: "/transaction" })
  .post(
    "/action",
    async ({ headers, body, request, set }) => {
      ...
      console.log(`[transaction-action] Starting processing for orderRef: ${orderRef}, payment_type: ${paymentType}`);
      ...
      try {
        ...
        console.log(`[transaction-action] Step 1: Splitting orderRef: ${orderRef}`);
        const parts = orderRef.split("-");
        const esCode = parts[0];
        console.log(`[transaction-action] Step 1 complete. Extracted esCode: ${esCode}`);
        ...
        console.log(`[transaction-action] Step 3 complete. Token successfully verified.`);
        ...
        console.log(`[transaction-action] Step 4 complete. Generated signature: ${calculatedSignature}`);

        // 5. ตรวจสอบและบันทึกข้อมูลลูกค้า (Check/Insert Customer)
        console.log(`[transaction-action] Step 5: Checking customer in custommer table...`);
        ...
        if (!existingCustomer) {
          console.log(`[transaction-action] Step 5: Customer not found. Inserting new customer...`);
          ...
          console.log(`[transaction-action] Step 5 complete. Inserted new customer with ID: ${customerDbId}`);
        } else {
          console.log(`[transaction-action] Step 5: Customer found. Updating customer details with ID: ${existingCustomer.id}...`);
          await sql`
          ...
          console.log(`[transaction-action] Step 5 complete. Updated customer details.`);
        }

        // 6. Update ลง table orders where order_ref
        console.log(`[transaction-action] Step 6: Updating orders table for order_ref: ${orderRef}...`);
        await sql`
          UPDATE "orders" SET
        ...
        `;
        console.log(`[transaction-action] Step 6 complete. Updated orders table.`);

        // 7. Insert ลง table order_items
        console.log(`[transaction-action] Step 7: Inserting items into order_items...`);
        ...
        console.log(`[transaction-action] Step 7 complete. Inserted all order items.`);
        ...
        console.log(`[transaction-action] Step 8: Sending POST request to ${fetchUrl}...`);
        ...
        try {
          ...
          console.error("Failed to log transaction-action internal error:", dbErr);
        }
        return responseBody;
      }
    },
