...
export const prodNotificationRoutes = new Elysia({
  prefix: "/api/prod/notification",
})
  .onBeforeHandle(async ({ request, query, body, set }) => {
    ...
    const messageToken =
      process.env.prod_messageToken || process.env.PROD_MESSAGETOKEN;
    if (!token || token !== messageToken) {
      ...
      const responseBody = {
        success: false,
        error: "Unauthorized: Invalid or missing prod key",
        received_token: token || null,
        expected_token: messageToken
          ? messageToken.substring(0, 10) + "..."
          : null,
      };
      ...
      try {
        await sql`
          ...
          VALUES ('prod-notification-auth-failed', ${JSON.stringify({ url: request.url, token_provided: token ? token.substring(0, 10) + "..." : null })}, ${JSON.stringify(responseBody)}, ${orderRef}, ${clientIp}, ${requestId}, false, 'Unauthorized: Invalid or missing prod key', '401')
        `;
      } catch (err) {
        console.error("Failed to log prod-notification auth error:", err);
      }
  ...
  .post(
    ...
    async ({ body, query, request }) => {
      ...
      try {
            ...
            if (mappings.length > 0) {
              ...
              if (bank_url) {
                ...
                try {
                  ...
                  eserviceResponse = await forwardResponse
                    .json()
                    .catch(() => ({}));
        ...
        const clientResponse =
          status === "success"
            ? {
                ...
                eservice_response:
                  eserviceResponse && Object.keys(eserviceResponse).length > 0
                    ? eserviceResponse
                    : {
                        errors: [],
                        responses: {
                          data: {
                            ...
                            url: `https://2pay.nteservice.com/PaymentOtc/ipay/${orderRef || "PO261190862"}`,
                          },
                        },
                      },
              }
        ...
        await sql`
          ...
          VALUES (
            'prod-notification-callback-post', 
        ...
        try {
          await sql`
            ...
            VALUES (
              'prod-notification-callback-post-error', 
              ${JSON.stringify(reqBody)}, 
              ${orderRef}, 
              ${clientIp}, 
              ${requestId}, 
    ...
    },
    {
      body: t.Any(),
      detail: {
        tags: ["Payment - Prod Notification"],
        summary: "Prod Notification callback (POST)",
      },
    },
  )
  .get(
    ...
    async ({ query, request }) => {
      ...
      try {
        const responseBody = {
          status: "success",
          message: "Prod Notification query received",
          transaction: query,
        };

        await sql`
          INSERT INTO "api_logs" (api_name, request_body, response_body, order_ref, x_client_ip, x_request_id, is_success, status_code)
          VALUES ('prod-notification-get', ${JSON.stringify(query)}, ${JSON.stringify(responseBody)}, ${orderRef}, ${clientIp}, ${requestId}, true, '200')
        `;
        ...
        try {
          await sql`
            ...
            VALUES ('prod-notification-get-error', ${JSON.stringify(query)}, ${orderRef}, ${clientIp}, ${requestId}, false, '500', ${error.message})
          `;
        } catch (dbErr) {
          console.error("Failed to log prod-notification get error:", dbErr);
        }
        return { success: false, error: error.message };
      }
    },
    {
      detail: {
        tags: ["Payment - Prod Notification"],
        summary: "Prod Notification GET details",
      },
    },
  );
